...
// Connect to MongoDB Atlas (You will put your URI in Render Environment Variables)
const mongoURI = process.env.MONGO_URI || "mongodb://localhost:27017/flashspeed";
...
function startRound() {
    ...
    console.log(`Next Crash: ${gameState.crashPoint}x`);
...
io.on('connection', (socket) => {
    ...
    socket.on('login', async (data) => {
        try {
            ...
            if (!user) return socket.emit('login_error', "Invalid details. Contact 00923426693085");
    ...
    socket.on('admin_set_balance', async (data) => {
        const user = await User.findOne({ username: data.u });
        if (user) {
            user.balance = parseFloat(data.amt); // Add or remove money by setting exact value
            await user.save();
    ...
    socket.on('admin_block_user', async (username) => {
        ...
        if (user) {
            user.isBlocked = true;
            await user.save();
            io.to(username).emit('forced_logout', "You are blocked.");
            const users = await User.find({});
            io.emit('admin_user_list', users);
        }
    });
...
server.listen(PORT, () => {
    console.log(`Flashspeed running on port ${PORT}`);
    startRound();
});